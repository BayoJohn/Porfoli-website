#!/bin/sh
echo "Token length: $(echo $GITEA_TOKEN | wc -c)"
git config --global user.email "drone@ci.local"
git config --global user.name "Drone CI"
git clone http://Bayo:$GITEA_TOKEN@172.17.0.1:3001/Bayo/Porfolio-website.git /tmp/repo
...
git -C /tmp/repo push http://Bayo:$GITEA_TOKEN@172.17.0.1:3001/Bayo/Porfolio-website.git main
